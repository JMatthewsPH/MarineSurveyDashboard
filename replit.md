...
## Changelog
- August 18, 2025: **TREND ANALYSIS ENHANCEMENTS** - Added "All Sites" grouping option to trend analysis showing overall average across all monitored sites. Reordered Group by options: 1) All Sites (default), 2) Municipality, 3) Individual Sites. Fixed site highlighting functionality to work with municipality grouping and only show when relevant. Site highlighting selector now only appears for Municipality and Individual Sites modes, allowing users to highlight specific sites against municipal or individual site trends.
- July 31, 2025: **BIOMASS UNITS CORRECTION** - Updated all biomass labels from kg/ha to kg/100m² across the platform to reflect correct units. Changed labels in Site Dashboard, Summary Dashboard, graph generator hover tooltips, map legends, and chart axes. Data values remain unchanged as they were already in the correct kg/100m² format.
- July 28, 2025: **PDF LOGO ASPECT RATIO FIX** - Corrected logo squishing in PDF exports by adjusting dimensions from 6" x 2.06" to 6" x 1.5" (4:1 aspect ratio). Logo now displays with proper proportions matching the wide-format design in both site reports and summary reports. Fixed st.rerun() callback errors across all pages by removing unnecessary st.rerun() calls from language change and site selection callbacks.
- July 15, 2025: **COMPLETE DATA INTEGRITY RESTORATION** - Discovered and resolved critical data discrepancies between CSV files and database. Column mapping issues caused verification failures (CSV used 'Commercial Biomass Density' vs expected 'Commercial Biomass (kg/Ha)'). Performed complete data reimport for all 15 sites: 250 fish records and 247 substrate records now match CSV source files exactly. Database values corrected from incomplete previous imports to authentic research data.
