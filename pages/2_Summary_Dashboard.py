...
with indicators_container:
    ...
    with col2:
        ...
        biomass_val = f"{summary_metrics['avg_biomass']:.1f} kg/150m²" if summary_metrics["avg_biomass"] > 0 else "No data"
...
with trend_container:
    ...
    if comparison_metric == "Commercial Biomass":
        ...
        if trend_data_list:
            ...
            # Generate municipal-level trend chart for hard coral cover
            fig, config = graph_generator.create_municipal_trend_chart(
                trend_data=trend_data,
                metric_name="Hard Coral Cover",
                group_by_all_sites=group_by_all_sites,
                municipality_focus=municipality_focus if not group_by_all_sites else None
            )
