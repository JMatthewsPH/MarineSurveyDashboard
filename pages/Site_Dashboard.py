...
zamboanguita_sites = sorted([site[1] for site in sites if site[2] == "Zamboanguita"])
siaton_sites = sorted([site[1] for site in sites if site[2] == "Siaton"])
santa_catalina_sites = sorted([site[1] for site in sites if site[2] == "Santa Catalina"])
...
if selected_site:
    selected_site_obj = next((site for site in sites if site[1] == selected_site), None)
    if selected_site_obj:
        ...
        site_municipality = selected_site_obj[2] if selected_site_obj else None
        ...
        with st.sidebar:
            ...
            try:
                ...
                if date_query and date_query.min_date and date_query.max_date:
                    ...
                    print(f"DEBUG: Expanded seasonal range from {date_query.min_date} to {date_query.max_date}, using {min_date} to {max_date}")
                else:
                    # Fallback dates if no data
                    min_date = pd.to_datetime('2017-03-01')  # Start of Spring 2017
                    max_date = pd.to_datetime('2025-08-31')  # End of Summer 2025
                    print(f"DEBUG: Using fallback seasonal date range from {min_date} to {max_date}")
                    
            except Exception as e:
                print(f"ERROR: Failed to get date range from database: {e}")
                # Fallback dates if query fails
                min_date = pd.to_datetime('2017-03-01')
            ...
            if biomass_comparison == "Compare with Sites":
                ...
                if biomass_compare_sites:
                    ...
                    site_to_muni = {site[1]: site[2] for site in sites}
